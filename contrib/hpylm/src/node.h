...
public:
    static id _auto_increment;						// Independent of VPYLM for identifier
    hashmap<id, Node*> _children;					// Child context tree
    hashmap<id, vector<int>> _arrangement;			// Placing customers, vector<int> Represents the number of customers in table k
    Node* _parent;									// Parent node
    int _num_tables;								// Total number of tables
    int _num_customers;								// Total number of customers
    int _stop_count;								// Number of stops. VPYLM use
    int _pass_count;								// Number os passes. VPYLM use
    int _depth;										// Depth of　root node is 0
    id _token_id;									// Word ID Character ID
    id _identifier;									// It has no special meaning for identification Unrelated to VPYLM
    ...
    void enumerate_tokens_of_each_depth(unordered_map<int, vector<vector<id>>> &tokens){
        vector<id> c_tok;
        c_tok.push_back(_token_id);
        for(auto &elem: _arrangement){
            c_tok.push_back(elem.first);
        }
        tokens[_depth].push_back(c_tok);
        for(auto &elem: _children){
            elem.second->enumerate_tokens_of_each_depth(tokens);
        }
    }
